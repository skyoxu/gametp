  ...
  async list(params?: RepositoryQueryParams): Promise<T[]> {
    ...
    if (params?.sortBy) {
      ...
      results.sort((a, b) => {
        ...
        const va = ra[sortBy!] as number | string | undefined;
        const vb = rb[sortBy!] as number | string | undefined;
        if (va === vb) return 0;
        if (va === undefined) return 1; // undefined moves to end
...
describe('Repository<T> contract tests', () => {
  ...
  describe('Concurrency', () => {
    it('detects and rejects conflicts', async () => {
      ...
      await expect(repository.upsert(stale as TestEntity)).rejects.toThrow(
        ConcurrencyError
      );
    });
    ...
    it('increments version correctly', async () => {
      ...
      await repository.upsert({ ...(v2 as TestEntity), name: 'Updated' });
