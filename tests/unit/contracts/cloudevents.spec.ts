  ...
  describe('assertCe - validator', () => {
    ...
    it('fails when required fields are missing', () => {
      ...
      badCases.forEach((evt, idx) => {
        expect(
          () => assertCe(evt as unknown as Record<string, unknown>),
          `case ${idx} should fail`
    ...
    it('fails when specversion is unsupported', () => {
      ...
      expect(() =>
        assertCe(invalid as unknown as Record<string, unknown>)
      ).toThrow("Unsupported CloudEvents specversion: 2.0, expected '1.0'");
    });
