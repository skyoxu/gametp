...
async function main() {
  ...
  if (!appId || !keyFile || !owner || !repo || !jobId) {
    console.error(
      'Usage: --app-id <id> --key-file <pem> --owner <o> --repo <r> --job-id <id> [--out file]'
    );
    process.exit(2);
  }
  ...
  if (tokenResp.status !== 201) {
    console.error(
      'access_tokens failed',
      tokenResp.status,
      tokenResp.json || tokenResp.body?.toString?.()
    );
    process.exit(4);
  }
