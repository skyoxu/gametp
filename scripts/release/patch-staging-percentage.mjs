 ...
 *
 * 功能：
 * - 修改 electron-updater feed 文件(latest.yml)中的 stagingPercentage（Windows-only）
 * - 支持 0-100% 范围的分阶段发布控制
 ...
 *
 * 参数：
 *   feedFile     - electron-updater feed 文件路径 (latest.yml)
...
function isValidFeedFile(feedFile) {
  return feedFile.endsWith('latest.yml');
}
