...
if (!CI) {
  console.log('fix-bom-src: CI=false, skip');
  process.exit(0);
}
...
const logFile = path.join(logDir, `fix-bom-src-${hh}${mi}${ss}Z.log`);
...
console.log(`fix-bom-src: checked ${checked}, fixed ${fixed} -> ${logFile}`);
