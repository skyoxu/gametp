...
if (tracked.stdout) {
  ...
  console.error(
    'The following files are tracked under dist/ or dist-electron/:'
  );
  console.error(tracked.stdout);
  console.error('\nHow to fix:');
  console.error(
    '  - Remove these files from git (git rm -r --cached dist dist-electron)'
  );
...
if (untracked.stdout) {
  console.log(
    '[guard-artifacts] Untracked build artifacts detected (OK if ignored):'
  );
  console.log(untracked.stdout);
}

console.log(
  '[guard-artifacts] OK: No tracked build artifacts under dist/ or dist-electron/.'
);
