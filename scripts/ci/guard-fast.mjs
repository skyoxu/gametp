...
async function detectPlaywright() {
  try {
    if (typeof import.meta.resolve === 'function') {
      const u = import.meta.resolve('@playwright/test');
...
async function main() {
  ...
  if (await detectPlaywright()) {
    ...
    if (
      String(process.env.FAST_EXTRA_SMOKE || '').toLowerCase() === '1' ||
      String(process.env.FAST_EXTRA_SMOKE || '').toLowerCase() === 'true'
    ) {
      steps.push({
        name: 'E2E Extra Smoke',
        cmd: 'npx',
        args: [
          'playwright',
          'test',
