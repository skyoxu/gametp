...
async function main() {
  ...
  const status = (v, w, f) => (f && v >= f ? '[FAIL]' : (w && v >= w ? '[WARN]' : '[OK]'));
  out.push(`\n- Start -> first window DOMContentLoaded: ${startToFirstPaint} ms ${status(startToFirstPaint, warnStart, failStart)}`);
  out.push(`- Scene switch (hash change proxy): ${sceneSwitch} ms ${status(sceneSwitch, warnScene, failScene)}`);

  // Main branch light gate with one retry if over FAIL threshold
  const isMain = (process.env.GITHUB_REF || '').toLowerCase() === 'refs/heads/main';
  const overFail = (failStart && startToFirstPaint >= failStart) || (failScene && sceneSwitch >= failScene);
  if (isMain && overFail) {
    ...
    await page2.evaluate(() => { window.location.hash = '#perf-smoke-retry'; });
    ...
    const stillFail = (failStart && start2 >= failStart) || (failScene && scene2 >= failScene);
    if (stillFail) {
      console.log(out.join('\n'));
      if (summary) {
        await import('node:fs/promises').then(fs => fs.writeFile(summary, out.join('\n') + '\n', { flag: 'a' }));
      }
      process.exit(1);
    }
  }
