...
for (const f of files) {
  ...
  const suspiciousRe =
    /(rm -rf\s+|\bgrep\b|\bsed\b|\bchmod\b|\bchown\b|\bln -s\b|\btail -f\b|\bhead -n\b|\bwhich\b|cut -d|\bawk\b)/i;
...
if (hits.length) {
  ...
  lines.push(
    'Tip: Prefer PowerShell code fences over bash for Windows-only docs.'
  );
} else {
...
if (suspicious.length) {
  ...
  lines.push(
    '说明：检测到 rm/grep/sed/awk/cut/ln/head/tail/which 等 Linux 命令，请补充 PowerShell 对应写法。'
  );
  lines.push(
    'Tip: See docs/maintainers/POWERSHELL_EQUIVALENTS.md for mapping.'
  );
}
...
console.log(
  `Scanned ${files.length} markdown files; bash fences: ${hits.length}`
);
